...
export function DashboardScreen({ navigation }: DashboardScreenProps) {
  ...
  const deviceRender = (device: Device) => (
    <BleDevice
      onPress={async (pickedDevice) => {
        setIsConnecting(true)
        // BLEService.connectToDevice('60:B7:6E:13:D0:08') //'9C:19:C2:1B:8E:7A'
        try {
          const res = await BLEService.connectToDevice(pickedDevice.id)
          console.log('res', res)
          if (res) {
            onConnectSuccess()
          }
  ...
  const scanDevices = (key: boolean) => {
    try {
      setFoundDevices([])
      BLEService.initializeBLE().then(() => BLEService.scanDevices(addFoundDevice, null, key))
      setTimeout(() => { BLEService.manager.stopDeviceScan() }, 500)
    } catch (e) {
      console.log(e)
